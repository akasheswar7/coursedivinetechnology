...
export const UserLogin = () => {
  ...
  // Always require fresh manual login when visiting the employee login endpoint
  useEffect(() => {
    logout();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#071328] flex flex-col justify-between text-slate-100 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[650px] h-[500px] bg-blue-500/15 rounded-full blur-[110px] animate-float-slow"></div>
      </div>
      ...
      <main className="flex-1 flex items-center justify-center p-4 relative z-10 animate-slide-up">
        <div className="w-full max-w-md bg-slate-900/85 backdrop-blur-2xl border border-white/15 rounded-3xl shadow-2xl p-8 sm:p-9 hover:border-blue-400/40 transition-colors duration-300">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-blue-400 mt-1">
              Admissions Counselor Workplace
            </p>
            <p className="text-xs text-slate-300 mt-1.5 font-medium">
              Sign in to access and manage your assigned student inquiries
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-slate-950/60 border border-slate-700/80 rounded-xl text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-mono"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-3 bg-slate-950/60 border border-slate-700/80 rounded-xl text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-mono"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full mt-3 py-3.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs sm:text-sm transition-all duration-200 shadow-lg hover:shadow-blue-500/25 flex items-center justify-center gap-2 disabled:opacity-50 hover:scale-[1.02] btn-shimmer"
            >
              {isLoading ? (
                <span>Verifying Account...</span>
              ) : (
                <>
                  <span>Sign In as Counselor</span>
          ...
          <div className="mt-6 p-3 rounded-2xl bg-slate-950/60 border border-slate-800/80 text-[11px] text-slate-400 text-center">
            🛡️ Strict Data Isolation: Each counselor only accesses their assigned leads.
          </div>
        </div>
      </main>
      ...
      <footer className="border-t border-white/10 bg-slate-950/60 backdrop-blur-md px-6 py-3 text-center text-xs text-slate-400 relative z-10">
        Course Divine Technology Pvt. Ltd. • Employee Workplace Security
      </footer>
    </div>
  );
};
